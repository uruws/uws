	...
	for i in pods['items']:
		...
		for cond in i['status'].get('conditions', {}):
			...
			if not sts['index'].get(typ, None):
				sts['index'][typ] = 0
			if not sts['cond'][ns][name].get(typ, None):
				sts['cond'][ns][name][typ] = 0
			if st == 'True':
				sts['index'][typ] += 1
				sts['cond'][ns][name][typ] += 1
	return sts
...
def config(sts):
	...
	for ns in sorted(sts['cond'].keys()):
		for name in sorted(sts['cond'][ns].keys()):
			...
			print('graph_scale yes')
			cc = 0
			for ctype in sorted(sts['cond'][ns][name].keys()):
